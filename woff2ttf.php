...
class WOFF2TTF
{
    ...
    public static function convert($woffFile, $convertPath)
    {
        if (is_null($woffFile) || is_null($convertPath)) {
            WOFF2TTF::bail("Empty argument: convert($woffFile, $convertPath");
        }
        ...
        if (! $fh = fopen($woffFile, "rb")) {
            WOFF2TTF::bail("Couldn't open file $woffFile.");
        }
            ...
            switch($key) {
                case 'sig':
                    ...
                    if ($debug) {
                        if ($val != 0x774F4646) { // wOFF
                            ...
                            WOFF2TTF::bail("File is not a valid WOFF font.", $fh);
                        } else {
                ...
                case 'res':
                    if ($val != 0) {
                        ...
                        WOFF2TTF::bail("Reserved field has to be 0 (zero).", $fh);
                    }
                    break;
                case 'ssize':
                    if ($debug) echo "\ntotal data size $val.";
        ...
        if (! $ofh) {
            WOFF2TTF::bail("Couldn't open file $outfile for writing.", $fh);
        }
        ...
        for ($i = 0; $i < $numTables; $i++ ) {
            ...
            if (fseek($fh, $dirEntry[1])) {
                WOFF2TTF::bail("fseek-ing to offset {$dirEntry[1]}", $fh, $ofh);
            }
